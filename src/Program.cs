...
if (args[0] == "anim-compile")
    return RunAnimCompile(args);
...
/// </summary>
// ── Anim-Compile: .anim.yaml → .hkx binary ──

static int RunAnimCompile(string[] args)
{
    if (args.Length < 2)
    {
        Console.Error.WriteLine("Usage: hkbuild anim-compile <input.anim.yaml> [-o <output.hkx>] [--fps <n>]");
        Console.Error.WriteLine();
        Console.Error.WriteLine("Compiles an animation YAML file to a Skyrim SE compatible .hkx binary.");
        Console.Error.WriteLine("Uses hkaSplineCompressedAnimation with THREECOMP40 rotation quantization.");
        Console.Error.WriteLine();
        Console.Error.WriteLine("Examples:");
        Console.Error.WriteLine("  hkbuild anim-compile mt_idle.anim.yaml -o mt_idle.hkx");
        Console.Error.WriteLine("  hkbuild anim-compile my_anim.anim.yaml --fps 60");
        return 1;
    }
    ...
    outputPath ??= Path.ChangeExtension(
        inputFile.EndsWith(".anim.yaml", StringComparison.OrdinalIgnoreCase)
            ? inputFile[..^".anim.yaml".Length]
            : inputFile,
        ".hkx");

    try
    {
        Console.WriteLine("HKBuild — Animation Compiler (YAML → HKX)");
        Console.WriteLine($"Reading:  {inputFile}");

        var anim = AnimationReader.Load(inputFile);

        Console.WriteLine($"Animation: {anim.Name}  duration={anim.Duration:F3}s  " +
                          $"tracks={anim.Tracks.Count}  floatTracks={anim.FloatTracks.Count}  fps={fps}");

        Console.Write("Compressing splines...");
        var compressed = SplineAnimCompressor.Compress(anim, fps);
        Console.WriteLine($" {compressed.NumFrames} frames, {compressed.NumBlocks} block(s), " +
        ...
        AnimationHkxEmitter.Emit(anim, compressed, fs, fps);
...
static void PrintUsage()
{
    ...
    Console.Error.WriteLine();
    Console.Error.WriteLine("  hkbuild anim-compile <input.anim.yaml> [-o <output.hkx>] [--fps <n>]");
    Console.Error.WriteLine("    Compile an animation YAML to a Skyrim SE HKX binary.");
    Console.Error.WriteLine();
    Console.Error.WriteLine("Examples:");
